...
interface TemplatesListPageProps {
  ...
  onNavigateToEditor: (template: EmailTemplate) => void;
  ...
  const headers = [
    ...
    { key: 'actions', label: 'Actions', render: (t: EmailTemplate) => (
      <div className="flex items-center justify-center space-x-2">
          <Button variant="ghost" size="sm" onClick={() => onCreateCampaignFromTemplate(t)} title="Create Campaign from Template" aria-label={`Create campaign from template ${t.name}`}>
              <RocketIcon className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setPreviewingTemplate(t)} title="Preview" aria-label={`Preview template ${t.name}`}>
              <EyeIcon className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => onNavigateToEditor(t)} title="Edit" aria-label={`Edit template ${t.name}`}>
              <PencilIcon className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => onDeleteTemplate(t.id)} title="Delete" aria-label={`Delete template ${t.name}`} className="text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400">
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-400">Templates can be saved from the campaign editor.</p>
      </div>
      <Card>
          ...
          {templates.length === 0 && (
              <div className="text-center py-10">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-700 dark:text-gray-300">No templates found</h3>
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Create a campaign and save its content as a template to get started.</p>
              </div>
          )}
      </Card>
